...
| `kb.setting(k text)`, `kb.set_setting(k text, v text)` | a store setting (`path_history` = `on` / `off`; `link_updates` = `off` / `report` / `rewrite`), NULL at its default; `kb.set_setting(k, NULL)` clears it; an unknown key or value is `TX004` |
