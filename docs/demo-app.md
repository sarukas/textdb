...
| **Import folder** | Pick a folder (Chrome and Edge read it lazily with the directory picker; other browsers take a whole-folder file input), choose the destination folder and file types, and watch progress. Unchanged files make no new version; binary, non-UTF-8, unreadable (for example online-only cloud files) and oversized files are skipped with the reason |
